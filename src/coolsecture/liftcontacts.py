...
def _make_result_db(path: Path) -> sqlite3.Connection:
    ...
    conn.execute(
        ...
        CREATE TABLE IF NOT EXISTS results (
            ...
            control_contacts INTEGER NOT NULL,
            observed_deviations INTEGER NOT NULL,
            control_deviations INTEGER NOT NULL,
            observed_coverages_pos1 INTEGER NOT NULL,
            observed_coverages_pos2 INTEGER NOT NULL,
            control_coverages_pos1 INTEGER NOT NULL,
            control_coverages_pos2 INTEGER NOT NULL,
            control_contact_distances REAL NOT NULL,
...
def _insert_differ_rows_sqlite(conn: sqlite3.Connection, differ: Dict) -> None:
    ...
    conn.executemany(
        ...
        INSERT OR REPLACE INTO results(
            n1,b1,n2,b2,remap1,remap2,observed_contacts,control_contacts,
            observed_deviations,control_deviations,observed_coverages_pos1,observed_coverages_pos2,
            control_coverages_pos1,control_coverages_pos2,control_contact_distances,remapping_coverages
        ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        """,
        rows,
    )
...
def _write_differ_from_sqlite(conn: sqlite3.Connection, Abins: Dict, out_prefix: str) -> None:
    ...
    with open(out_path, 'w', buffering=1024 * 1024) as f:
        ...
        cur = conn.execute(
            """
            SELECT n1,b1,n2,b2,remap1,remap2,observed_contacts,control_contacts,
                   observed_deviations,control_deviations,observed_coverages_pos1,observed_coverages_pos2,
                   control_coverages_pos1,control_coverages_pos2,control_contact_distances,remapping_coverages
            FROM results
...
def run_liftover(
        ...
        with _tmp_workspace(tmp_dir) as td:
            ...
            try:
                ...
                for shard_path in shard_paths:
                    ...
                    DifferA, _, Statistic = _iDifferContact_core(
    ...
    if nthreads_val > 1:
        ...
        DifferA, _ = iDifferContact_parallel(
...
def main():
    ...
    p = argparse.ArgumentParser(
        prog='liftcontacts',
        description='Compatibility wrapper. Prefer `coolsecture liftcontracts`.',
    ...
    print("[WARN] 'liftcontacts' is a compatibility wrapper. Forwarding to 'liftcontracts'.")
