...
def _target_key_from_row(row, order, res):
...
def _is_reciprocal_from_ab_row(row_ab, key_a, ba_rows, order_a, order_b, res_ab, res_ba):
    key_b = _target_key_from_row(row_ab, order_b, res_ab)
    if key_b is None:
        return False
    row_ba = ba_rows.get(key_b)
    if row_ba is None:
        return False
    back_key = _target_key_from_row(row_ba, order_a, res_ba)
        ...
        try:
            ...
            with open(lift_ab, "r", buffering=1024 * 1024) as f:
                ...
                for line in f:
                    ...
                    key_b = _target_key_from_row(row, order_b, res_ab)
                    is_consistent = False
                    if key_b is not None:
                        ...
                        if row_ba is not None:
                            back_key = _target_key_from_row(list(row_ba), order_a, res_ba)
