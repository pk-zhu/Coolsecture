...
def _stream_contacts_to_sqlite(
    ...
    sql = (
        "INSERT INTO contacts(i,j,c1,b1,c2,b2,obs,ctl,l) VALUES(?,?,?,?,?,?,?,?,?) "
        "ON CONFLICT(i,j) DO UPDATE SET "
        "c1=excluded.c1,b1=excluded.b1,c2=excluded.c2,b2=excluded.b2,"
        "obs=excluded.obs,ctl=excluded.ctl,l=excluded.l "
    ...
    with open(liftover_path, "r", buffering=1024 * 1024) as f:
        ...
        for line in f:
            ...
            buf.append((i, j, cc1, bb1, cc2, bb2, obs, ctl, l))
            processed += 1
            if len(buf) >= 100000:
...
def _write_juicer_input_from_sqlite(path: str, conn: sqlite3.Connection, res: int, which: str):
    col = "obs" if which == "observed" else "ctl"
...
def _run_single(
            ...
            try:
                ...
                if fmt in ("cool", "both"):
                    ...
                    try:
                        ...
                        uri_ctl = f"{out_prefix}.Control.cool"
                        _write_cool_from_sqlite(uri_obs, bins_df, conn, "obs", assembly=assembly, tmp_dir=tmp_dir)
                        _write_cool_from_sqlite(uri_ctl, bins_df, conn, "ctl", assembly=assembly, tmp_dir=tmp_dir)
                ...
                if fmt in ("hic", "both"):
                    ...
                    in_ctl = os.path.join(td, "control.txt")
                    ...
                    _write_juicer_input_from_sqlite(in_ctl, conn, res, "control")
                    out_obs = f"{out_prefix}.Observed.hic"
                    out_ctl = f"{out_prefix}.Control.hic"
                    _run_juicer_tools(juicer_tools, in_obs, out_obs, chrom_sizes, res)
                    _run_juicer_tools(juicer_tools, in_ctl, out_ctl, chrom_sizes, res)
                    print(f"[OK] wrote {out_obs}")
                    print(f"[OK] wrote {out_ctl}")
            finally:
                conn.close()
        return
    ...
    if fmt in ("cool", "both"):
        ...
        try:
            ...
            _write_cool_minimal(uri_ctl, bins_df, px_ctl, assembly=assembly)
            print(f"[OK] wrote {uri_obs}")
            print(f"[OK] wrote {uri_ctl}")
        except Exception as e:
            print(f"Warning: Failed to generate .cool files due to dependency issue: {e}")
    ...
    if fmt in ("hic", "both"):
        with _tmp_workspace(tmp_dir) as td:
            ...
            _write_juicer_input(in_ctl, contacts, res, "control")
            out_obs = f"{out_prefix}.Observed.hic"
            out_ctl = f"{out_prefix}.Control.hic"
            _run_juicer_tools(juicer_tools, in_obs, out_obs, chrom_sizes, res)
            _run_juicer_tools(juicer_tools, in_ctl, out_ctl, chrom_sizes, res)
...
def main():
    p = argparse.ArgumentParser(
        prog="lift2matrix",
        description="Create observed/control matrices from .liftContacts (cool or hic)",
