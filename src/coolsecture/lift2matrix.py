...
def _collect_chrom_name_map(liftover_path: str, chrom_alias_map: dict):
    mapping = {}
    with open(liftover_path, "r", buffering=1024 * 1024) as f:
        ...
        for line in f:
            ...
            if len(a) < 4:
                continue
            for name in (a[0], a[2]):
                mapped = canonicalize_chrom_name(name, chrom_alias_map)
                if mapped is not None:
                    mapping[name] = mapped
    return mapping


def _write_chrom_name_map(path: str, mapping: dict):
    changed = {k: v for k, v in mapping.items() if k != v}
    if not changed:
        print("[INFO] chromosome names unchanged")
        return
    with open(path, "w") as f:
        f.write("input_name\tfasta_name\trename_applied\n")
        for input_name in sorted(mapping):
            fasta_name = mapping[input_name]
            f.write(f"{input_name}\t{fasta_name}\t{str(input_name != fasta_name).lower()}\n")
    print(f"[WARN] chromosome name aliases were used; see {path}")
...
def _run_single(
    ...
    chrom_name_map = _collect_chrom_name_map(liftover_path, chrom_alias_map)
    _write_chrom_name_map(f"{out_prefix}.chrom_name_map.tsv", chrom_name_map)
