...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--resolution", "-res", default=None,
        help="Resolution (bp). For .hic input, required single value. For .mcool, use comma-separated list for multi-res.")
...
def main():
    ...
    if res_vals:
        if is_hic_path(cool_path):
            if len(res_vals) != 1:
                raise RuntimeError("--resolution for .hic input must be a single value.")
            hic_res = res_vals[0]
        else:
            res_list = res_vals
    ...
    if res_list:
        if "::resolutions/" in cool_path:
            raise RuntimeError("--resolution expects a .mcool path without ::resolutions/RES")
        if not cool_path.lower().endswith(".mcool"):
            raise RuntimeError("--resolution only supports .mcool input")
        summary_rows = []
