...
def main():
    p = argparse.ArgumentParser(
        prog="similarity",
        description="Compute stratum-adjusted correlation coefficient (SCC) between matched source-coordinate matrices (typically Observed vs Control from lift2matrix)",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument("--matrix-a", required=True, help="Observed matrix (lift2matrix outputs)")
    p.add_argument("--matrix-b", required=True, help="Control matrix (lift2matrix outputs)")
