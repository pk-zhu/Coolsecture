...
def main():
    p = argparse.ArgumentParser(description="Compute windowed contact metrics from a .liftContacts table and write bedGraph")
    ...
    p.add_argument("--random-stats", action="store_true", help="Plot randomized baseline KDE")
    ...
    for frame in args.frames:
        ...
        if args.random_stats:
            rnd = randomize_contacts(contacts, idx=1)
            Rr = metricCalc(rnd, res, frame=frame, metric=args.metric, max_dist_mb=args.max_dist_mb)
            rnd_vals = np.array([x[3] for x in Rr if len(x) == 4 and np.isfinite(x[3])], dtype=float)
        ...
        if obs_vals.size == 0:
            ...
            if rnd_vals is not None and rnd_vals.size > 0:
                xs = np.linspace(vmin, vmax, 400)
                try:
                    from sklearn.neighbors import KernelDensity
                    kde = KernelDensity(kernel='gaussian', bandwidth=(vmax-vmin)/40.0).fit(rnd_vals[:,None])
                    ys = np.exp(kde.score_samples(xs[:,None]))
                except Exception:
                    h, edges = np.histogram(rnd_vals, bins=bins, range=(vmin, vmax), density=True)
